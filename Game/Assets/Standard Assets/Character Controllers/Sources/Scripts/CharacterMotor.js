...
var lightDelay : int;
var lightMaxDelay : int;
var timerDone : boolean;
var intensityDropNum : int;
...
function Update () {
	...
	var dLight : Light;
	dLight = GameObject.Find("d_light").light;	
	...
	if (!timerDone) {
		...
		if(restSeconds == 2 || restSeconds == 1){
			if(intensityDropNum % 3 == 1){
		...
		lightDelay = Time.time - startTime;
	}
	
	if (Input.GetKeyDown(KeyCode.L)) {
		
		if (restSeconds <= 0 && lightDelay >= lightMaxDelay){
			Debug.Log("Light Pressed and Started");
			...
			Debug.Log("Light Pressed, but Delay active");
		}	
    }
